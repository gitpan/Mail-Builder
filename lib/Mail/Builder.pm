...
Each call to this method also chages the L<messageid>.

=head3 purge_cache

Emptys the attachment and image cache and removes the plaintext text if it has
been autogenerated from html text. Also deletes the Message ID.
...
available only after the C<build_message> or C<stingify> methods have been 
called. Each call to C<build_message> or C<stingify> changes the message id.
...
enginges mark html e-mail messages without a plaintext part as spam.
...
Enables the autogeneration of plaintext parts from HTML. Default TRUE.
